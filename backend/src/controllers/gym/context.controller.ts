...
export const syncContext = async (req: Request, res: Response) => {
  try {
    ...
    if (error || !data) {
      return res.status(404).json({ error: 'Módulo gym no encontrado' })
    }

    if (data.tipo_modulo !== 'gym') {
      return res.status(400).json({ error: 'El módulo encontrado no es de tipo gym' })
    }
