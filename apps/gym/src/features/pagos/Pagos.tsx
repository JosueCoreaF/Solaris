  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 14, marginBottom: 24 }}>
        {[
          { label: 'Ingresos del Mes', value: fmt(totalMes), color: '#16a34a' },
          { label: 'Total Pagos', value: pagos.filter(p => p.estado !== 'anulado').length, color: '#2563eb' },
          { label: 'Pagos Anulados', value: pagos.filter(p => p.estado === 'anulado').length, color: '#dc2626' },
        ].map(c => (
          <div key={c.label} className="stat-card">
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 8 }}>{c.label}</div>
              ...
              {pagos.map(p => {
                ...
                return (
                  <tr key={p.id_pago_gym}>
                    <td style={{ fontWeight: 600 }}>{insc?.miembros?.nombre_completo ?? '—'}</td>
                    <td>{insc?.planes_membresia?.nombre ?? '—'}</td>
                    <td style={{ fontWeight: 700, color: 'var(--accent)' }}>{fmt(p.monto)}</td>
                    ...
                    <td>
                      {p.estado !== 'anulado' && (
                        <button style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#dc2626', fontSize: 11, padding: '3px 8px', borderRadius: 6 }}
