...
import {
  ...
  getAllRecetasByPlatillos, createRecetaItem, deleteRecetaItem,
} from '../api/inventario';
...
export const Inventario: React.FC = () => {
  ...
  const [addToPlatilloId, setAddToPlatilloId] = useState('');
  ...
  const load = async () => {
    ...
    setLoading(false);
    if (pl.length > 0) {
      loadAllRecetas(pl.map((x: Platillo) => x.id_platillo));
    }
  };

  useEffect(() => { load(); }, [restaurant]);

  const loadAllRecetas = async (ids: (string | number)[]) => {
    setLoadingRecetas(true);
    const r = await getAllRecetasByPlatillos(ids);
    setAllRecetas(r as any);
  ...
  const handleAddReceta = async () => {
    if (!recetaForm.id_inventario || !addToPlatilloId) { setErrorReceta('Selecciona un ingrediente.'); return; }
    if (recetaForm.cantidad_utilizada <= 0) { setErrorReceta('La cantidad debe ser mayor a 0.'); return; }
    setSavingReceta(true);
    try {
      await createRecetaItem({ id_platillo: addToPlatilloId, id_inventario: recetaForm.id_inventario, cantidad_utilizada: recetaForm.cantidad_utilizada });
      await loadAllRecetas(platillos.map((x: Platillo) => x.id_platillo));
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        {/* botón de recetas se mueve a cada tarjeta */}
      </div>
          ...
          <>
            ...
            {loadingRecetas ? (
              ...
              <div className="text-center py-16 text-slate-500 text-sm">
                ...
                <p>No hay platillos registrados. Crea platillos primero.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {platillos
                  .filter(p => p.nombre_platillo.toLowerCase().includes(recetaSearch.toLowerCase()))
                  .map(platillo => {
                    const recetasDelPlatillo = allRecetas.filter(
                      r => String(r.id_platillo) === String(platillo.id_platillo)
                    );
                    return (
                      <motion.div
                        ...
                        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden hover:border-orange-500/40 transition-colors"
                      >
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-slate-400">
                                {recetasDelPlatillo.length === 0
                                  ? 'Sin ingredientes'
                                  : `${recetasDelPlatillo.length} ingrediente${recetasDelPlatillo.length > 1 ? 's' : ''}`}
                              </p>
                            </div>
                          </div>
                          <button
                            onClick={() => {
                              setAddToPlatilloId(String(platillo.id_platillo));
                              setRecetaForm({ id_inventario: '', cantidad_utilizada: 1 });
                              setErrorReceta(null);
                              setRecetaModal(true);
                            }}
                            className="flex items-center gap-1 text-xs text-orange-400 hover:text-orange-300 border border-orange-500/30 hover:border-orange-500/60 px-2 py-1 rounded-lg transition-colors flex-shrink-0"
                          >
                        ...
                        <div className="px-4 py-3">
                          {recetasDelPlatillo.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-4 text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
                              <p className="text-xs">Sin receta definida</p>
                            </div>
                          ) : (
                            <ul className="space-y-1.5">
                              {recetasDelPlatillo.map((r: any) => (
                                <li key={r.id_rec_platillo} className="flex items-center justify-between gap-2 group">
                                  <div className="flex items-center gap-2 min-w-0">
                                    <div className="w-1.5 h-1.5 bg-orange-400 rounded-full flex-shrink-0" />
                                    <span className="text-sm text-slate-700 dark:text-slate-300 truncate">
                                      {r.inventario?.producto?.nombre_producto ?? `Ingrediente #${r.id_inventario}`}
                                    </span>
                                  </div>
                                  <div className="flex items-center gap-2 flex-shrink-0">
                                    ...
                                    <button
                                      onClick={() => handleDeleteReceta(r.id_rec_platillo)}
      ...
      <Modal open={recetaModal} onClose={() => setRecetaModal(false)} title={`Agregar ingrediente — ${platillos.find(p => String(p.id_platillo) === addToPlatilloId)?.nombre_platillo ?? ''}`} size="sm">
        <div className="space-y-4">
          <div>
